...
public class HistorialMedico {
    ...
    public List<EntradaHistorial> construirLineaDeTiempo(List<Turno> turnosAtendidos, List<Vacunacion> vacunaciones) {
        ...
        for (Turno turno : turnosAtendidos) {
            ...
            String diagnosticos = turno.getDetalles().stream()
                .map(d -> d.getServicio())
                .filter(s -> s instanceof Consulta)
                .map(s -> (Consulta) s)
                .filter(c -> c.getDiagnostico() != null && !c.getDiagnostico().isBlank())
                .map(c -> "Diagnóstico: " + c.getDiagnostico()
                    + (c.getTratamiento() != null && !c.getTratamiento().isBlank()
            ...
            entradas.add(new EntradaHistorial(
                ...
                detalle + " - Dr. " + turno.getVeterinario().getNombre() + diagnosticos));
        }
